...
const userSchema = new mongoose.Schema({
  ...
  // Social links
  socialLinks: [{
    platform: { type: String, enum: ['twitter', 'github', 'linkedin', 'instagram', 'facebook', 'youtube', 'website'] },
    url: { type: String }
  }],
...
userSchema.methods.addXP = async function(amount, reason) {
  ...
  // Log XP gain
  console.log(`User ${this.email} gained ${amount} XP for: ${reason}. Total XP: ${this.xp}`);
...
userSchema.statics.getLeaderboard = function(type = 'xp', limit = 50) {
  ...
  return this.find().sort({ [sortField]: -1 }).limit(limit);
};
