  ...
  const mockResults = [
    {
      ...
      excerpt: 'Discover how artificial intelligence is revolutionizing the way we create, edit, and distribute content across various platforms. From automated writing assistants to intelligent content optimization...',
    ...
    },
    {
      ...
      excerpt: 'Learn the strategies and techniques needed to create engaging blog series that keep readers coming back for more. This comprehensive guide covers everything from planning to execution...',
      author: 'Michael Chen',
    ...
    },
    {
      ...
      excerpt: 'Explore how speech recognition technology is changing the landscape of content creation and making writing more accessible to everyone. Discover the latest tools and techniques...',
    ...
    },
    {
      ...
      excerpt: 'How game mechanics and rewards systems are transforming the way we learn and retain information. This post explores the psychology behind gamification and its applications...',
      author: 'David Kim',
    ...
    },
    {
      ...
      excerpt: 'Understanding what makes content go viral and how to create posts that truly resonate with your audience. Deep dive into the psychological principles that drive engagement...',
    ...
    },
    {
      ...
      excerpt: 'Practical tips and strategies for reducing your environmental footprint through everyday choices and habits. Learn how small changes can lead to significant positive impact...',
      author: 'Alex Green',
  ...
  const performSearch = async () => {
    ...
    try {
      ...
      let filtered = mockResults.filter(result => {
        ...
        const matchesLanguage = !filters.language || result.language === filters.language;
        const matchesReadTime = !filters.readTime || 
          (filters.readTime === '0-5' && result.readTime <= 5) ||
          (filters.readTime === '5-10' && result.readTime > 5 && result.readTime <= 10) ||
          (filters.readTime === '10-15' && result.readTime > 10 && result.readTime <= 15) ||
          (filters.readTime === '15+' && result.readTime > 15);
        
        return matchesQuery && matchesMood && matchesLanguage && matchesReadTime;
      });
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        <h1 className="text-4xl font-bold text-text-primary">Search VocalInk</h1>
        <p className="text-lg text-text-secondary">Discover amazing content from our community</p>
      </div>
          ...
          <form onSubmit={handleSearch} className="space-y-4">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-11 pr-4 h-12 text-lg"
              />
              <Button 
                type="submit" 
              ...
              <div className="flex flex-wrap gap-2">
                {suggestions.map((suggestion, index) => (
                  <button
                    ...
                    className="px-3 py-1 text-sm bg-secondary-100 text-text-secondary rounded-full hover:bg-secondary-200 transition-colors"
                  >
                    {suggestion}
                  </button>
                ))}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          {hasActiveFilters && (
            <Button variant="ghost" onClick={clearFilters} className="text-error hover:text-error">
              Clear all
            </Button>
          )}
        </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {moods.map((mood) => (
                      ...
                      >
                        ...
                        <div className="text-xs font-medium text-text-primary">{mood.name}</div>
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {searchResults.length > 0 ? (
            <div className={`grid gap-6 ${
              ...
              {searchResults.map((result) => (
                <Card key={result.id} className="hover:shadow-lg transition-all duration-300 cursor-pointer">
                  <CardContent className="p-6">
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <div>
                            <div className="font-medium text-text-primary">{result.author}</div>
                            <div className="text-sm text-text-secondary">
                              {new Date(result.publishedAt).toLocaleDateString()}
                            </div>
                          </div>
                        </div>
                        <Badge className={moods.find(m => m.id === result.mood)?.color}>
                          {moods.find(m => m.id === result.mood)?.icon} {moods.find(m => m.id === result.mood)?.name}
                        </Badge>
                      </div>
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {trendingTopics.map((topic, index) => (
                <div key={index} className="p-4 border border-border rounded-lg hover:bg-surface transition-colors cursor-pointer">
