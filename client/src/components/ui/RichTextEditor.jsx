...
import { Undo2, Redo2, Bold, Italic, List, ListOrdered, Code, Quote, Link2, Link2Off, Image as ImageIcon, Table as TableIcon, Type } from 'lucide-react';
...
const RichTextEditor = ({ value, onChange, className = '' }) => {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose max-w-none focus:outline-none text-text-primary leading-8 text-[17px] font-serif',
      },
  ...
  const insertImage = async () => {
    const method = window.prompt('Type "url" to insert by URL, or "upload" to upload an image', 'upload');
    if (!method) return;
    ...
    <div className={`border border-border rounded-lg ${className}`}>
      <div className="p-2 lg:p-3 border-b border-border flex flex-wrap items-center gap-2 text-sm sticky top-0 bg-background z-10">
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1 text-xs text-text-secondary"><Type className="w-3.5 h-3.5" /> Text</span>
          <select
        ...
        {divider}
        <div className="flex items-center gap-1">
          <button title="Bold" onClick={() => editor.chain().focus().toggleBold().run()} className={btn(editor.isActive('bold'))}><Bold className="w-4 h-4" /></button>
          <button title="Italic" onClick={() => editor.chain().focus().toggleItalic().run()} className={btn(editor.isActive('italic'))}><Italic className="w-4 h-4" /></button>
          <button title="Bullet List" onClick={() => editor.chain().focus().toggleBulletList().run()} className={btn(editor.isActive('bulletList'))}><List className="w-4 h-4" /></button>
          <button title="Ordered List" onClick={() => editor.chain().focus().toggleOrderedList().run()} className={btn(editor.isActive('orderedList'))}><ListOrdered className="w-4 h-4" /></button>
          <button title="Code Block" onClick={() => editor.chain().focus().toggleCodeBlock().run()} className={btn(editor.isActive('codeBlock'))}><Code className="w-4 h-4" /></button>
          <button title="Quote" onClick={() => editor.chain().focus().toggleBlockquote().run()} className={btn(editor.isActive('blockquote'))}><Quote className="w-4 h-4" /></button>
        </div>
        {divider}
        <div className="flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_220px] gap-4">
        <div className="p-6 min-h-64 bg-white text-black rounded-lg border border-border">
          <EditorContent editor={editor} className="text-black leading-8 text-[17px]" style={{ whiteSpace: 'pre-wrap' }} />
        </div>
        <aside className="hidden lg:block p-3 border border-border rounded-lg h-fit sticky top-20 bg-background">
          <div className="text-xs uppercase tracking-wide text-text-secondary mb-2">Headings</div>
          <ul className="space-y-1 text-sm text-text-primary">
            {headings.length === 0 && <li className="text-text-secondary">No headings yet</li>}
            {headings.map((h, idx) => (
              <li key={`${h.pos}-${idx}`}>
                <button
